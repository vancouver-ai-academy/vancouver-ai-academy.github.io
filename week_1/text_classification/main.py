...
def trainval(exp_dict, savedir, args):
    ...
    # (TODO) Step 1: Get train_loader for the yelp_review_full dataset from huggingface
    # Load the yelp_review_full dataset and create a DataLoader for it
    dataset = None
    train_loader = None

    # (TODO) Step 2: Load Tokenizer and 'distilbert/distilbert-base-uncased' Model and Optimizer
    model = None
    opt = None
...
if __name__ == "__main__":
    ...
    if args.exp_group == "text_classifier":
        exp_list = []
        for lr in [1e-3, 1e-4]:
            exp_list += [
                {"lr": lr, "dataset": "yelp_review_full", "model": "distillbert"}
            ]
